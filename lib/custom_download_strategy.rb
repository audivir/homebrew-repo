...
# GitHubPrivateRepositoryTarballDownloadStrategy downloads automatically-generated
# tarballs from GitHub Releases. To use it, add `:using => :github_private_tarball`
# to the URL section of your formula. This download strategy uses GitHub access
# tokens (in the environment variables HOMEBREW_GITHUB_API_TOKEN) to sign the request.
class GitHubPrivateRepositoryTarballDownloadStrategy < GitHubPrivateRepositoryDownloadStrategy
  def initialize(url, name, version, **meta)
    super
  end
  ...
  def _fetch(url:, resolved_url:, timeout:)
    # HTTP request header `Accept: application/octet-stream` is required.
    # Without this, the GitHub API will respond with metadata, not binary.
    curl_download download_url, "-L", "--header", "Authorization: Bearer #{@github_token}", to: temporary_path
  end
  ...
  class << self
    module Compat
      ...
      def detect_from_symbol(symbol)
        ...
        when :github_private_tarball
          GitHubPrivateRepositoryTarballDownloadStrategy
        when :s3
          S3DownloadStrategy
        when :scp
          ScpDownloadStrategy
